 ...
 *
 * The rules live here rather than in the analyser so they can be tested without a camera,
 * and so tuning a threshold is a domain change with a failing test to prove it — not an
 * edit inside a frame callback.
 */
class EvaluateFaceAlignmentUseCase @Inject constructor() {

    operator fun invoke(faces: List<FaceObservation>): FaceAlignment = when {
