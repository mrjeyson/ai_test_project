package com.example.test_ai_project.home.presentation.map.screen
...
import com.example.test_ai_project.home.presentation.map.viewmodel.MapViewModel
...
internal fun MapScreen(
    ...
    val permissionLauncher = rememberLauncherForActivityResult(
        ...
        onResult = { grants ->
            viewModel.onEvent(MapEvent.PermissionResult(grants.values.any { it }))
        },
    )
    ...
    LaunchedEffect(Unit) {
        when {
            ...
            context.hasLocationPermission() ->
                viewModel.onEvent(MapEvent.PermissionResult(true))
    ...
    MapScreen(
        uiState = uiState,
        onRecentre = { viewModel.onEvent(MapEvent.RefreshRequested) },
        onCameraSettled = { camera -> viewModel.onEvent(MapEvent.CameraSettled(camera)) },
        onRequestPermission = { permissionLauncher.launch(LocationPermissions) },
        onDismissMessage = { viewModel.onEvent(MapEvent.MessageDismissed) },
        modifier = modifier,
    )
}
...
internal fun MapScreen(
    ...
    Box(modifier = modifier.fillMaxSize()) {
        GoogleMap(
            ...
            contentDescription = stringResource(id = ResR.string.map_content_description),
        ...
        Column(modifier = Modifier.fillMaxWidth().align(Alignment.TopCenter)) {
            ...
            if (uiState.messageRes != null) {
                MapBanner(
                    ...
                    actionRes = ResR.string.map_retry,
                    onAction = onRecentre,
                    onDismiss = onDismissMessage,
                )
            } else if (uiState.permission == LocationPermission.Denied) {
                MapBanner(
                    messageRes = ResR.string.map_permission_body,
                    actionRes = ResR.string.map_permission_action,
        ...
        FloatingActionButton(
            ...
            if (uiState.isLocating) {
                ...
                Icon(
                    ...
                    contentDescription = stringResource(id = ResR.string.map_recentre),
                )
            }
        }
    }
...
private fun UserMarker(location: UserLocation) {
    ...
    Marker(
        ...
        title = stringResource(id = ResR.string.map_marker_title),
        snippet = location.accuracyMeters?.let { accuracy ->
            stringResource(id = ResR.string.map_marker_accuracy, accuracy.toInt())
        },
    )
}
        ...
        Row(
            ...
            if (onDismiss != null) {
                TextButton(onClick = onDismiss) {
                    Text(text = stringResource(id = ResR.string.map_dismiss))
                }
            }
        }
    }
...
private fun CachedBadge(modifier: Modifier = Modifier) {
    Surface(
        ...
        Row(
            ...
            Text(
                text = stringResource(id = ResR.string.map_cached_badge),
 ...
 */
private fun MapState.startPosition(): CameraPosition {
