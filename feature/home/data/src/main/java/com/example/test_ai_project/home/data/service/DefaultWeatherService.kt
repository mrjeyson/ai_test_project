...
import com.example.test_ai_project.home.data.mapper.mapWeather
 ...
 */
@Singleton
class DefaultWeatherService @Inject constructor(
    ...
    /**
     * Acquiring the fix and fetching the weather are one operation from the screen's point
     * of view, so they are one call here — a screen that had to sequence them itself would
     * be re-implementing this in every tab that shows the weather.
     */
    override suspend fun refresh(relocate: Boolean) {
        ...
        refreshFor(location = location, force = relocate)
    }

    private suspend fun refreshFor(location: UserLocation, force: Boolean) =
